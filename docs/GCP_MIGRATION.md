...
After owner-approved source data deletion, the managed source database returns empty application results and its embedded preview fails to load. The managed task controls expose Preview, Code, Dashboard, Database, File storage, and Settings, but no automated unpublish or project-delete control is available through the current task interface. The source environment no longer serves project data; permanent project/deployment account removal must be completed through the owner’s management controls if desired.

The owner then confirmed source unpublishing. The managed deployment has been switched to **Not published**, and the former `condocore-zqdgaya5.manus.space` address is no longer publicly launched. Google Cloud is the only published production endpoint.

Final cutover verification returned HTTP 404 for the former managed source URL and HTTP 200 with `{"ok":true}` from `http://136.68.220.42/healthz`. The temporary HTTP endpoint is intentionally retained at the owner’s direction; custom-domain HTTPS remains a future enhancement rather than a migration blocker.
